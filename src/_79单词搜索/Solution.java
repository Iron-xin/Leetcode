...
public class Solution {
    public boolean exist(char[][] board, String word) {
        ...
        if (board.length*board[0].length < arr.length)
            return false;
    ...
    //进行代码的合并
    public boolean dfs1(char[][] board, boolean[][] visited, char[] arr, int i, int j, int depth) {
        ...
        //可以将上面四句和并成一个
        if(dfs(board, visited,arr, i - 1, j, depth + 1)
                || dfs(board, visited,arr, i + 1, j, depth + 1)
                || dfs(board, visited,arr, i, j - 1, depth + 1)
                || dfs(board, visited,arr, i, j + 1, depth + 1))
            return true;
        // 返回上一层递归栈，需要将当前节点置为未访问过
        visited[i][j] = false;
